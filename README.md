...
Next.jsを使ったシンプルなマップアプリケーションです。
OpenStreetMapを使用してインタラクティブな地図を表示します。

## 技術スタック

- **Next.js 16** - React フレームワーク（App Routerを使用）
- **React 19** - ユーザーインターフェース構築
- **TypeScript** - 型安全性
- **Tailwind CSS 4** - スタイリング
- **Leaflet / react-leaflet** - マップライブラリ
- **SQLite** - データベース（better-sqlite3）
- **ESLint** - コード品質管理

## 機能

- OpenStreetMapを使用したインタラクティブなマップ表示
- 東京をデフォルト表示位置として設定
- マーカーとポップアップのサポート
- レスポンシブデザイン対応
- ダークモード対応（手動切替機能付き）
...
1. リポジトリをクローン：
    ```bash
    git clone https://github.com/TakuyaFukumura/map-next-js-app.git
    ```
    ```bash
    cd map-next-js-app
    ```
